    ...
    _createClass(Drawer, [{
      ...
      value: function open(event) {
        ...
        jQuery.getJSON('/cart.js', function(cart) {
          ...
          if(cart_total < 1000){
            $('.shipping-bar').remove();
            var bar_value = (cart_total * 100) / 1000
            $('<div class="shipping-bar"><progress max="100" value="'+bar_value+'" style="width: 100%;margin-top: 15px;accent-color: #000"></progress><marquee direction="left">Free Shipping above order value of Rs.1000</marquee></div>').insertBefore('.Cart__ItemList')
          }
        });
...
//         document.dispatchEvent(new CustomEvent('theme:loading:start'));
        ...
        if($('.ProductForm__Item').length > 0 && !$('.sticky-bottom').hasClass('show')){
          ...
          var blazer_size = '';
          if(meta.product.type == 'Shirt' || meta.product.type == 'Designer Shirt' || meta.product.type == 'Jacket'){
...
//            check variant is available or not
              ...
              if(!$(this).hasClass('option-selected')){
                is_size_selected = false;
				if($('.size-error').length < 1){
                  if(meta.product.type == 'Shirt Trouser'){
                    var option1_msg = '<p class="size-error">Please select Shirt Size</p>';
                    var option2_msg = '<p class="size-error">Please select Pant Size</p>';
                  }else if(meta.product.type == 'Shirt Short'){
                    var option1_msg = '<p class="size-error">Please select Shirt Size</p>';
                    var option2_msg = '<p class="size-error">Please select Short Size</p>';
                  }else if(meta.product.type == 'Shirt Lounge Pant'){
                    var option1_msg = '<p class="size-error">Please select Shirt Size</p>';
                    var option2_msg = '<p class="size-error">Please select Lounge Pant Size</p>';
                  }else if(meta.product.type == 'Tshirt Short'){
                    var option1_msg = '<p class="size-error">Please select Tshirt Size</p>';
                    var option2_msg = '<p class="size-error">Please select Short Size</p>';
                  }else if(meta.product.type == 'Tshirt Lounge Pant'){
                    var option1_msg = '<p class="size-error">Please select Tshirt Size</p>';
                    var option2_msg = '<p class="size-error">Please select Lounge Pant Size</p>';
                  }else{
                    var option1_msg = option2_msg = '<p class="size-error">Please select Size</p>';
                  }
                  var errorMessageElement = document.createElement('span');
                  errorMessageElement.className = 'ProductForm__Error Alert Alert--error';
                  if(index == 0){
                  	errorMessageElement.innerHTML = option1_msg;
                  }else{
                  	errorMessageElement.innerHTML = option2_msg;
                  }
                  addToCartButton.setAttribute('disabled', 'disabled');
                  addToCartButton.insertAdjacentElement('afterend', errorMessageElement);
